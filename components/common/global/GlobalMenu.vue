	...
	<div id="header">
		<nav class="gnb_wrap">
			...
			<div class='gnb'>
                ...
                <ul>
                    <a v-if='containMain' href='/main' id="gnav-main" class="home "></a>
                    <li v-for='(item, index) in menuList' id='item.id' :key='index'> 
                        <a  class="menuStyle btnCursor" href='item.href'> <span>{{item.text}}</span>  </a>
                    </li>
                </ul>
			</div>
		</nav>
...
export default {
    ...
    data(){
        return{
            containMain : false, 
            menu_gismap : { id : 'menu_gismap', href : '/statusMap', text : '계통지리정보'},

        ...
        <div class="btn_sel_wrap">
            <div class="top_selBox_zone">
                <span style='padding-right: 12rem;'>
                    <input @click='getdata(null)' value='null' type="radio" name="selRadio" id="selRadio1" class="selBox_radio">
                    <label for="selRadio1" >
                        <div style=' padding-left:0.5rem;' class="btn_sel_wrap">
                        <dl style='width: 4rem;'><dt style='line-height: 2rem;'>{{ $t('manage.all') }}</dt></dl></div>
                    </label>
                </span>
                <span style='padding-right: 12rem;'>
                    <input @click='getdata(1)' value='1' type="radio" name="selRadio" id="selRadio2" class="selBox_radio">
                    <label for="selRadio2">
                        <div style=' padding-left:0.5rem;' class="btn_sel_wrap">
                        <dl  style='width: 4rem;'><dt style='line-height: 2rem;'>{{ $t('manage.auth_approval') }}</dt></dl></div>
                    </label>
                </span>
                <span style='padding-right: 12rem;'>
                    <input @click='getdata(0)' value='0' type="radio" name="selRadio" id="selRadio3" class="selBox_radio">
                    <label for="selRadio3">
                        <div style='width: 10rem; padding-left:0.5rem;' class="btn_sel_wrap">
                        <dl  style='width: 5rem;'><dt style='line-height: 2rem;'>{{ $t('manage.auth_disapproval') }}</dt></dl></div>
                    </label>
                </span>
            </div>
        </div>
            ...
            <table border="0" cellpadding="0" cellspacing="0" summary="">
                ...
                <thead>
                    <tr>
                        ...
                        <th  style="width:45rem !important">{{ $t('manage.address') }}</th>
                        <th  style="width:15rem !important">{{ $t('manage.companyname') }}</th>
                        <th  style="width:10rem !important">{{ $t('manage.registdate') }}</th>
                        <th  style="width:10rem !important">{{ $t('manage.expiredate') }}</th>
                        <th  style="width:10rem !important">{{ $t('manage.auth_approval') }}</th>
                ...
                <tbody class="tableScrollbar">
                    <tr v-for="v in bodycomponent" :key="v.idx">
                        ...
                        <td  style="width:14rem !important" @click='popupopen(v.idx)' v-html='v.data10'>{{v.data10}}</td>
					...
					<div class="cont1">
						<div class="tbl_wrap">
							...
							<caption></caption>
								...
								<tbody>
									<tr>
										<th><span class="tbl_th">{{ $t('manage.name') }}</span></th>
										<td><span class="tbl_td"><input type="text" id="username" name="" value="" class="pop_input" disabled /></span></td>
									</tr>
									<tr>
										<th><span class="tbl_th ">{{ $t('manage.id') }}</span></th>
										<td><span class="tbl_td"><input type="text" id="email" name="" value="" class="pop_input" disabled/></span></td>
									</tr>
									<tr>
										<th><span class="tbl_th ">{{ $t('manage.companyname') }}</span></th>
										<td><span class="tbl_td"><input type="text" id="company" name="" value="" class="pop_input" disabled/></span></td>
									</tr>
                                    <tr>
										<th><span class="tbl_th ">{{ $t('manage.level') }}</span></th>
										<td><span class="tbl_td inner_search"><input type="text" id='authtype' class="pop_input" disabled/></span></td>
									</tr>
                                    <tr>
										<th><span class="tbl_th ">{{ $t('manage.registdate') }}</span></th>
										<td><span class="tbl_td inner_search"><input type="text" id='regist' class="pop_input" disabled/></span></td>
									</tr>
                                    <tr>
										<th><span class="tbl_th ">{{ $t('manage.expiredate') }}</span></th>
                                        <td>
                                            <span class="tbl_td inner_search">
                                                <input type="text" id='expire' class="pop_input"  style='width:13rem; margin-right: 1rem;'/>
                                                <div class=" alR">
                                                    <button style='right: 0rem' id='done' type="button" @click="setexpire('done')" class="setexpire btn btn_size_free"><span>즉시 만료</span></button> 
                                                </div>
                                            </span>
                                        </td>
									</tr>
									<tr>
										<th><span class="tbl_th ">{{ $t('manage.address') }}</span></th>
										<td>
                                            <span class="tbl_td inner_search"><input type="text" id='seladdress' class="pop_input" />
                                            <button @click='DaumPostcode()' type="button" class="btn btn_search"></button></span>
                                        </td>
									</tr>
									<tr>
										<th><span class="tbl_th">{{ $t('manage.tel') }}</span></th>
										<td><span class="tbl_td"><input type="text" id='seltel' class="pop_input" /></span></td>
                                    </tr>
                                    <tr>
										<th><span class="tbl_th">사업자등록증</span></th>
										<td>
                                            <button type="button" @click='downloadcompanydoc()' class="btn btn_size_free btn_color_blue">
                                                <span>{{ $t('manage.download') }}</span></button> 
                                           
                                        </td>
                                    </tr>
                                    <tr>
										<th><span class="tbl_th">{{ $t('manage.auth_approval') }}</span></th>
										<td>
						...
						<div class="btn_wrap alR">
							<button @click ='save()' type="button" class="btn btn_size_free btn_color_blue"><span>{{ $t('manage.save') }}</span></button>
...
export default {
    ...
    methods: {
        downloadcompanydoc(){
            ...
            var url = 'https://www.peiu.co.kr:3020/api/information/v1/download/registdocument/' + this.popupuserid

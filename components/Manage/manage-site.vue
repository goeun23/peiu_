<template>
  <div id="sitemanagement" class="componentofmenu market_record_wrap">
    ...
    <div
      class="admin_section"
      style="float: left; width: 42%; margin-right: 5rem"
    >
      <ul class="sitemange">
        <li>
          ...
          <span class="txt">
            <select
              v-model="aggid"
              @change="setcontractorlist($event)"
              class="sel sel_size1"
            >
          ...
          <span class="txt" style="padding-left: 0.5rem">
            ...
            >
              <option value=""></option>
              <option v-for="v in constractorlist" :key="v.idx" :value="v.id">
                {{ v.value }}
              </option>
            </select>
          </span>
        ...
        <li>
          <span class="tit">사이트 번호</span>
          <span class="txt">
            <input
              @click="empty()"
              v-on:keyup.13="checkinvalidsiteid"
              type="text"
              class="input_txt"
              id="input_siteid"
            />
            <button
              @click="checkinvalidsiteid()"
            ...
            <span
              style="padding-left: 0.5rem; color: #ec0037"
              id="isinvalidsiteid"
              class="txt"
            ></span>
          </span>
        ...
        <li>
          ...
          <span class="txt"
            ><input type="text" id="lat" class="pop_input" readOnly
          /></span>
        </li>
        <li>
          <span class="tit">경도</span>
          <span class="txt"
            ><input type="text" id="lng" class="pop_input" readOnly
          /></span>
        </li>
        <li>
          <span class="tit">법정동코드</span>
          <span class="txt"
            ><input type="text" id="bcode" class="pop_input" readOnly
          /></span>
        </li>
        <li>
          ...
          <span class="top_selBox_zone">
            <span style="padding-right: 12rem">
              ...
              <label>
                <div style="padding-left: 0.5rem" class="btn_sel_wrap">
                  <dl style="width: 4rem">
                    <dt style="line-height: 2rem; vertical-align: top">허용</dt>
                  </dl>
                </div>
              </label>
            </span>
            <span style="padding-right: 12rem">
              ...
              <label>
                <div style="padding-left: 0.5rem" class="btn_sel_wrap">
                  <dl style="width: 6rem">
                    <dt style="line-height: 2rem; vertical-align: top">
                      비허용
                    </dt>
                  </dl>
                </div>
    ...
    >
      <ul class="sitemange">
        <li>
          <span class="tit">설비 정보</span
          ><span class="txt"
            >설비 대수를 입력하고 생성 버튼을 선택하여 용량, 설치 일자를
            추가하세요</span
          >
        </li>
        <li><span class="tit"></span><span class="txt"></span></li>
        <li>
          ...
          <span class="txt">
            <span class="input_option_control_wrap">
              ...
              <button type="button" class="btn_option_control control_up">
                <span></span>
              </button>
              <button type="button" class="btn_option_control control_down">
                <span></span>
              </button>
            </span>
          </span>
          <span style="padding-left: 4rem; width: 6rem" class="tit"
            >설치일자</span
          >
          ...
          <button
            style="margin-left: 4rem; width: 7rem"
            @click="createTbltr()"
            type="button"
            class="btn btn_sel"
          >
                ...
                <div class="divice_tbl tbl_wrap">
                  <table id="device-info" class="tbl_error_info">
                    ...
                    <thead>
                      <tr>
                        <td class="">
                          <span class="tit" style="width: 10rem"></span>
                        </td>
                        <td class="">
                          <span class="tit" style="width: 15rem">설비</span>
                        </td>
                        <td class="">
                          <span class="tit" style="width: 15rem">번호</span>
                        </td>
                        <td class="">
                          <span class="tit" style="width: 15rem">용량(kW)</span>
                        </td>
                        <td class="">
                          <span class="tit" style="width: 15rem">설치일자</span>
                        </td>
                      </tr>
                    </thead>
                    <tbody>
        ...
        <li>
          ...
          <span class="txt">
            <button
              style="margin-left: 4rem; width: 11.5rem"
              type="button"
              class="btn btn_sel"
            >
              <span>사이트 등록</span>
...
export default {
  head: {
    link: [],
    script: [{ src: "/scripts/data/latlngbylawcode.js" }],
  },
  data() {
    return {
      OptionList: [],
  ...
  methods: {
    checkauth(v) {
      // 0 허용 1 미허용
      $("input:radio[name='controlauth']:radio[value='0']").prop(
        "checked",
        true
      );
    },
    ...
    createTbltr() {
      ...
      var innerHtml = "";
      innerHtml +=
        "<tr rowid='device_row' id=" +
        type +
        indexnum +
        " class='completed'>" +
        "<td><span><input type='checkbox' data-device-id=" +
        type +
        indexnum +
        " name='device_check' id=" +
        type +
        indexnum +
        " class='tbl_inner_check'><label for='tbl_check0'></label></span></td>" +
        "<td class='info3' id=" +
        type +
        indexnum +
        "-device style='width:15rem; text-align : center;'><span>" +
        type +
        "</span></td>" +
        "<td class='info3' id=" +
        type +
        indexnum +
        "-indexnum style='width:15rem; text-align : center;'><span>" +
        indexnum +
        "</span></td>" +
        "<td class='info3' style='width:15rem; text-align : center;'><span><span><input type='text' style='width:10rem;' id=" +
        type +
        indexnum +
        "-capacity  value=" +
        $("#device_capacity").val() +
    ...
    setcontractorlist(e) {
      ...
      if (this.aggid != "") {
        var query =
          "SELECT * FROM mysql.peiu_account.`vw_contractorusers` where AggGroupId = '" +
          this.aggid +
          "'";
  ...
  mounted() {
    ...
    var siteid = _query(
      "select SiteId from mysql.peiu_account.`vw_contractorsites`"
    );
    this.checkauth(0);

...
// var basicLength =0;
// let aniX, aniY;
let fromRccX,fromRccY,toRccX,toRccY
// let myDist = 0;
// let mySeg = 0;
// let aniIdx = 0;
let routeRccWay=[];
...
let lineRccFunction = d3.line()
    .x(function (d) {  return d.x + 80; })
    .y(function (d) { return d.y; })  
let fromRccNodeMap = new HashMap();
let toRccNodeMap = new HashMap();
...
function drawRcc(rccData, diagramDivName) 
{
  ...
  // var barWidth = parseInt(dWidth /(eleTotalCnt*.2));
  var barWidth = parseInt(dWidth/(eleTotalCnt / 4.5));
  var barHeight = 4;
  var bar1Y = (dHeight / 3).toFixed(0) * 1
  var bar2Y = (dHeight / 3).toFixed(0) * 2;
  var singleBarW = parseInt((barWidth * 5));
...
////////////////////수평 데이터 세팅//////////////////////////////
  ...
  for(var i=0; i<linkedJson.length; i++){ 
    json = linkedJson[i];
    rt = _fromtoRccXY(barJson, json.from, json.to);
    // console.log(json.from+' ->' +json.to)
    linkPath.push({x : rt.fromRccXY.x, y : rt.fromRccXY.y})
    linkPath.push({x : rt.toRccXY.x, y : rt.toRccXY.y})
    rccSvg.append("path")
      .attr("d", lineRccFunction(linkPath))
      .attr("stroke", "#7F9FFF")
      .attr("stroke-width", 2)
  ...
  for (var i = 0; i < hBarJson.length; i++) {
    ...
    if (barObj.category == "circleTemplate") {
      ...
      // else if(barObj.key.includes("tieBar-CB"))
...
{ 
  ...
  for(var i=0; i< linkedJson.length; i++)
  {
      ...
      if(fromRccNodeMap.get(link.from) == null){ 
          fromRccNodeMap.set(link.from, link.to);
      }else{
          var tmp = [];
          var f = fromRccNodeMap.get(link.from);
          ...
          fromRccNodeMap.set(link.from, tmp);
      }

      if(toRccNodeMap.get(link.to) == null){
          toRccNodeMap.set(link.to, link.from);
      }else{
          var tmp = [];
          var f = toRccNodeMap.get(link.to);
          tmp.push(f);
          tmp.push(link.from);
          toRccNodeMap.set(link.to, tmp);
      }
  }
  ...
  for(var i=0; i<linkedLine.length-1; i++)
  {
    ...
    rt = _fromtoRccXY(barJson, fromName, toName);
    fromRccX = rt.fromRccXY.x;
    fromRccY = rt.fromRccXY.y;
    toRccX = rt.toRccXY.x;
    toRccY = rt.toRccXY.y;
    linkPath.push({x : fromRccX, y : fromRccY})
    linkPath.push({x : toRccX, y : toRccY})
    ...
    aniLineGroup.append("path").attr('id','aniPath')
      ...
      if(i == 0)      
      { 
        aniPath.push({x : fromRccX, y : fromRccY});
      } 

      if(i+2 < linkedLine.length)
      {
        var nextName = linkedLine[i+2];
        rt = _fromtoRccXY(barJson, nextName, null);
        var nextX = rt.fromRccXY.x;
        var nextY = rt.fromRccXY.y;
        if(fromRccX != nextX && fromRccY != nextY)
        {
          aniPath.push({x : toRccX, y : toRccY})
        }
      }else
      {
        aniPath.push({x : toRccX, y : toRccY})
  ...
  { 
    ...
    .text('◀') 
    // .attr('id','text-'+ _i)
    .attr('transform', 'translate('+(_start.x + 80 - (_fontSize/2))+ ' ' 
          + (_start.y + (0))+') rotate(' + _angle + ')')
    // .attr('fill','yellow')
...
function _routing(barJson,  from ,to){
  ...
  var isOpen = true;
    ...
    routeRccWay.push(from);     

    //to 에 있는지 조회
    node = toRccNodeMap.get(from); 
...
function _fromtoRccXY(barJson, from, to){
  var fromRccXY;
  var toRccXY;
  ...
  for (var i = 0; i < barJson.length; i++) {
    if (from == barJson[i].key) {
      fromRccXY = { x: barJson[i].loc.barX, y: barJson[i].loc.barY };
      break;
    }
  }
  for (var i = 0; i < barJson.length; i++) {
    if (to == barJson[i].key) {        
      toRccXY = { x: barJson[i].loc.barX, y: barJson[i].loc.barY };
      break;
    }
  }
  return {fromRccXY : fromRccXY, toRccXY : toRccXY};
}

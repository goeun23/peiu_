...
<!-- cont_body -->
    <div class="cont_body">
        <div class="tab_wrap size1" style=" background : #1A2031 !important; margin-top : 2.6rem !important; margin-left: 0rem !important; margin-right: 0rem !important;">
            <div class="tab_tit">
                <ul>
                    <li class="is_active" @click="selectSearch('stat')">
                        <a class="btn_tab_tit">
                            <span>발전예측</span>
                        </a>
                    </li>
            ...
            <div class="tab_cont">
                ...
                <div class="top_selBox_zone" style="padding: 0 0 0 0rem !important;">
                        <div class="sel_set_wrap">
                            <select id='evtCategory' @change='SetCategory($event)' class="sel sel_size1">
                                <option value="criteria" selected="selected">{{$t("statstics.criteria")}}</option>
                                <option value="rcc" >{{$t("manage.area")}}</option>
                                <option value="agg" >{{$t("manage.agg")}}</option>
                                <option value="site" >자원</option>
                            </select>
					    </div>
                        <div class="sel_set_wrap aggforsiteid">
                            <select id="rccaggcategory" @change="setselectId($event)" class="sel sel_size1">
                                <option value="rccaggcriteria" selected="selected">{{criteria}}</option>
                                <option v-for="v in OptionList" :key='v.idx' :value='v.id'>{{v.value}}</option>
                            </select>
                        </div>
                        <!-- siteid 선택하기 -->
                        <div  class="sel_set_wrap site">
                            <select id="siteidcategory" @change="setSiteId($event)"  style="width:22.5rem;"  class="sel sel_size1">
                                <option value="siteidcriteria" selected="selected">자원</option>
                            ...
                            <dl>
                                ...
                                <dd>
                                     <span class="pick_calendar_wrap">
                                        <input autocomplete="off" id="startDate" type="text" class="pick_calendar" readonly /> 
                                    </span>	~
                                    <span class="pick_calendar_wrap">
                                       
                                        <input autocomplete="off" id="endDate" type="text" class="pick_calendar" readonly />
                                    </span>	
                                    <button @click="setToday()"  type="button" class="btn btn_sel"><span>금일</span></button>
                                    <button @click="getStatChart()" type="button" class="btn btn_search"><span class="hide">찾기</span></button>
                                    <button @click="downloadCSV()" type="button" class="btn btn_down"><span class="hide">다운받기</span></button>
                                </dd>
                            </dl>
                        ...
                        <div class="">
                            ...
                            <div class="fixed_tbl_wrap" >
                                <div class="tbl_wrap record_tbl">
                                    <table id="t" border="0" cellpadding="0" cellspacing="0" summary="">
                                        <thead>
                                            <tr>
                                                <th style="width:13rem;" >
                                                    <button  @click="prev()" type="button" class="btn" style='font-color:white;'><span>◁</span></button>
                                                    {{date}} 
                                                    <button  @click="next()" type="button" class="btn" style='font-color:white;'><span>▷</span></button>
                                                </th>
                                                <th style="text-align:center; width:7rem;" v-for='v in thead' :key="v.hour">
                                                    <span>{{v.hour}}</span>
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody style="height: 12.5rem;">
                                            <tr>
                                                <td style="width:12.5rem;" ><span>예측값</span></td>
                                                <td style=" text-align:center;  width:7rem;"  v-for='v in tbody_table' :key="v.index">
                                                    <span>{{v.forcast}}</span>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td style="width:12.5rem;" ><span>실제값</span></td>
                                                <td style=" text-align:center; width:7rem;"  v-for='v in tbody_table' :key="v.index">
                                                    <span>{{v.pvgen}}</span>
                                                </td>
                                            </tr>
                                            ...
                                            <tr>
                                                <td style="width:12.5rem;" ><span>오차율</span></td>
                                                <td style=" text-align:center; width:7rem;"  v-for='v in tbody_table' :key="v.index">
                                                    <span>{{v.error}}</span>
                                                </td>
                                            </tr>
...
export default {
	...
	data(){
		return{
			...
			url_MasterAggGroupID : "https://www.peiu.co.kr:3021/api/contractor/getcontractorsites?aggregatorgroupid=",
		}
	},
	head: {
        link: [ 
	...
	methods: {
        downloadCSV(){
            ...
            if(this.category == "" || this.category == null){
                ...
                if(filtering == "" || filtering == "rccaggcriteria" ||filtering == "siteidcriteria" || filtering == null){
                    errorpoup('선택 값 없음')
                    this.emptyselectedfield('data')
        ...
        getChartData(){
            var item = _wherecondition(this.category, this.selectedItem, this.selectedsiteId)
            var p = " FROM mysql.peiuoperation.`vwHourlyPvGenerations` where " + item + " and createts >='"+ this.startTS +"' and createts <= '" + this.endTS +"'"
            
            if(this.category == 'rcc' || this.category == 'agg'){
                var send = "SELECT createts, sum(forecastpvgeneration) as forcast, sum(sumofpvgeneration) as pvgen " + p + " group by createts";
            }else if(this.category == 'site'){
                var send = "SELECT createts, forecastpvgeneration as forcast, sumofpvgeneration as pvgen" + p
            }
            ...
            // 설치용량 
            var facility = getInstalledData(this.category, this.selectedItem, this.selectedsiteId)
		...
		getStatChart(){
            ...
            this.datelist = this.getdatearray($("#startDate").val(), $("#endDate").val(), this.datelist)
        ...
        setTblData(){
            ...
            var item = _wherecondition(this.category, this.selectedItem, this.selectedsiteId)
            var p = " FROM mysql.peiuoperation.`vwHourlyPvGenerations` where " + item + " and createts >='"+ start +"' and createts <= '" + end +"'"

  ...
  <div class="dashboard_wrap">
    <div class="dashboard_box current">
      <strong class="title active"
        ><a href="#">기능로그 <i class="xi-angle-up" aria-hidden="true"></i></a
      ></strong>
    ...
    <div class="dashboard_box">
      <strong class="title"
        ><a href="#">메뉴로그 <i class="xi-angle-up" aria-hidden="true"></i></a
      ></strong>
      <div class="conts active">
  ...
  methods: {
    getPreviouse1: function () {
      if (this.offset1 - this.limit1 >= 0) {
        ...
        this.func_log_data = [];
        for (
          var index = this.offset1;
          index < this.recvFuncLog.length && index < this.offset1 + this.limit1;
          index++
        ) {
    ...
    getNext1: function () {
      ...
      if (this.offset1 >= this.recvFuncLog.length) {
        ...
        for (
          var i = this.offset1;
          i < this.recvFuncLog.length && i < this.offset1 + this.limit1;
          i++
        ) {
    ...
    getPreviouse2: function () {
      if (this.offset2 - this.limit2 >= 0) {
        ...
        this.menu_log_data = [];
        for (
          var index = this.offset2;
          index < this.recvMenuLog.length && index < this.offset2 + this.limit2;
          index++
        ) {
    ...
    getNext2: function () {
      ...
      if (this.offset2 >= this.recvMenuLog.length) {
        ...
        for (
          var i = this.offset2;
          i < this.recvMenuLog.length && i < this.offset2 + this.limit2;
          i++
        ) {
    ...
    getFuncLogResult: function (data, textStatus, jqXHR) {
      if (textStatus == "success") {
        if (data.rows.length > 0) {
          ...
          for (
            var index = 0;
            index < this.recvFuncLog.length && index < this.limit1;
            index++
          ) {
            var value = this.recvFuncLog[index];
            this.func_log_data.push(value);
    ...
    getMenuLogResult: function (data, textStatus, jqXHR) {
      if (textStatus == "success") {
        if (data.rows.length > 0) {
          ...
          for (
            var i = 0;
            i < i < this.recvMenuLog.length && i < this.limit2;
            i++
          ) {
            var value = this.recvMenuLog[i];
            this.menu_log_data.push(value);
...
.pagination1 div.active {
  background-color: #4caf50;
  color: white;
}
.pagination1 div:hover:not(.active) {

        ...
        <div class="top_selBox_zone">
          <div class="sel_set_wrap">
            <select
              ...
              class="sel sel_size1"
            >
              <option value="criteriabase" selected="selected">
                {{ $t("statstics.criteria") }}
              </option>
          ...
          <div class="sel_set_wrap">
            <select
              style="width: 10rem"
              id="selectrccaddid"
              @change="setselectId($event)"
              class="sel sel_size1"
            >
              <option vaule="initselectid" value="null" selected="selected">
                {{ selectedcategory }}
              </option>
              <option v-for="v in OptionList" :key="v.idx" :value="v.id">
                {{ v.value }}
              </option>
            </select>
          </div>
          <div class="sel_set_wrap site">
            <select
              style="width: 22.5rem"
              @change="setsiteid($event)"
              id="selectsiteid"
              class="sel sel_size1"
            >
              <option value="initsite" selected="selected">자원</option>
              <option v-for="v in SiteList" :key="v.idx" :value="v.siteid">
                {{ v.represenation }}
              </option>
            </select>
          </div>
          ...
          <div v-if="isismart" class="sel_set_wrap" style="margin-left: 0.9rem">
            <span style="font-size: 1.4rem; color: #ffffff" class="text">
              항목 :
            </span>
            <select
              @change="setIsmartContent($event)"
              style="padding-right: 0.9rem; width: 10.5rem"
              class="sel sel_size1"
            >
          ...
          <div class="btn_sel_wrap">
            <dl>
              ...
              <dd>
                ...
                <button @click="setToday()" type="button" class="btn btn_sel">
                  <span>{{ $t("statstics.today") }}</span>
                </button>
                <button
                  @click="Search('period', null)"
                  type="button"
                  class="btn btn_search"
                >
                  <span class="hide">찾기</span>
                </button>
                <button @click="download()" type="button" class="btn btn_down">
                  <span class="hide">다운받기</span>
                </button>
                <button
                  id="30min"
                  @click="searchbytime('30min')"
                  type="button"
                  class="s_bytime btn btn_sel"
                >
                  <span>30분전</span>
                </button>
                <button
                  id="1hour"
                  @click="searchbytime('1hour')"
                  type="button"
                  class="s_bytime btn btn_sel"
                >
                  <span>1시간전</span>
...
export default {
  ...
  methods: {
    ...
    _statusData() {
      ...
      if (this.selectedbtn == "equip") {
        ...
        if (this.category == "rcc" || this.category == "agg") {
          get =
            "SELECT distinct createts, sum(activepower) over (partition by createts) as actpwr ,avg(soc) over (partition by createts) as soc, sum(pvgeneration)over (partition by createts) as pvgen";
        } else if (this.category == "site") {
          get =
            "SELECT createts, activepower as actpwr, pvgeneration as pvgen, soc";
        }
    ...
    getHistoryData() {
      ...
      var offset =
        " OFFSET " +
        (500 * (this.pagination - 1) + (this.pagination - 1)) +
        " ROWS";

      var where = _wherecondition(
        this.category,
        this.selectId,
        this.selectedsiteId
      );
      ...
      switch (this.selectedbtn) {
        case "equip":
          select +=
            "fromtable.createts as createts, con.RCC as rcc, con.AggName as aggname, con.SiteId as siteid, con.ServiceCode as servicecode, fromtable.activepower as actpwr, fromtable.pvgeneration as pvgen, soc, soh ";
          from += "`vwMinuteMeasurements` as fromtable";
          siteid = "siteid";
          break;
        case "forcast":
          select +=
            "fromtable.createts as createts, con.rcc as rcc, con.AggName as aggname, con.SiteId as siteid, sumofpvgeneration as pvgen, forecastpvgeneration as forcast";
          from += "`vwHourlyPvGenerations`as fromtable";
          siteid = "SiteId";
          break;
        case "event":
          select +=
            "con.RCC as rcc, con.AggName as aggname, con.SiteId as siteid, DeviceType, DeviceIndex, Name, fromtable.createts as createts, Recoveryts , Ackts";
      ...
      var join =
        " JOIN mysql.peiu_account.`vw_contractorsites` as con ON fromtable." +
        siteid +
        " = con.SiteId";
      var page =
        maxpage + from + join + " WHERE fromtable." + where + start + end;
      var query =
        select +
        from +
        join +

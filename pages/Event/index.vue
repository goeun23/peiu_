	...
	<div class="cont_body" style="background : #1A2031 !important; ">
		...
		<div class="top_event_state_zone">
			<dl>
				...
				<dd>
					<ul>
                        <li><button id='0' @click="showRegionEvt(0,'전국')" type="button" class="area_name all_area"><span class="hide">전국</span></button></li>
						<li><button id='1' @click="showRegionEvt(1,'경기')"  type="button" class="noevent area_name" >{{$t("locale.local4")}}</button></li>
						<li><button id='2'  @click="showRegionEvt(2,'인천')"  type="button" class="noevent area_name" >{{$t("locale.local3")}}</button></li>
						<li><button id='3'  @click="showRegionEvt(3, '서울')"  type="button" class="noevent area_name" >{{$t("locale.local1")}}</button></li>
						<li><button id='4'  @click="showRegionEvt(4, '대전')"  type="button" class="noevent area_name" >{{$t("locale.local7")}}</button></li>
						<li><button id='5'  @click="showRegionEvt(5, '충남')"  type="button" class="noevent area_name" >{{$t("locale.local5")}}</button></li>
						<li><button id='6'  @click="showRegionEvt(6, '전북')"  type="button" class="noevent area_name" >{{$t("locale.local10")}}</button></li> 
						<li><button id='7'  @click="showRegionEvt(7, '전남')"  type="button" class="noevent area_name" >{{$t("locale.local2")}}</button></li><!-- 이벤트 발생이 없을경우 disabled -->
						<li><button id='8'  @click="showRegionEvt(8, '광주')"  type="button" class="noevent area_name" >{{$t("locale.local9")}}</button></li>
						<li><button id='9'  @click="showRegionEvt(9, '강원')"  type="button" class="noevent area_name" >{{$t("locale.local6")}}</button></li>
						<li><button id='10'  @click="showRegionEvt(10, '충북')"  type="button" class="noevent area_name" >{{$t("locale.local8")}}</button></li>
						<li><button id='11'  @click="showRegionEvt(11, '경북')"  type="button" class="noevent area_name" >{{$t("locale.local14")}}</button></li>
						<li><button id='12'  @click="showRegionEvt(12, '대구')"  type="button" class="noevent area_name" >{{$t("locale.local11")}}</button></li>
						<li><button id='13'  @click="showRegionEvt(13, '울산')"  type="button" class="noevent area_name" >{{$t("locale.local12")}}</button></li>
						<li><button id='14'  @click="showRegionEvt(14, '부산')"  type="button" class="noevent area_name" >{{$t("locale.local15")}}</button></li>
						<li><button id='15'  @click="showRegionEvt(15, '경남')"  type="button" class="noevent area_name" >{{$t("locale.local13")}}</button></li>
						<li><button id='16'  @click="showRegionEvt(16, '제주')"  type="button" class="noevent area_name" >{{$t("locale.local16")}}</button></li>
					</ul>
				</dd>
			</dl>
		</div>
		...
		<div class="top_event_info_tbl">
			<dl>
				<dt>{{$t("locale.selected")}} : <span id="Region" class="sel_area">{{Region}}</span></dt>
				<dd>
					<div class="fixed_tbl_wrap">
						<p class="tbl_topR_comment btn_comment"><button @click='SendAck()' type="button" class="btn btn_size_free btn_color_blue"><span>ACK</span></button></p>
						<div class="tbl_wrap tbl_event_info">
							...
							<caption></caption>
								...
								<thead >
									<tr>
										<th>
											<input @click="allEvtAck" type="checkbox" id="tbl_checkAll" name="aaaa" value="" class="tbl_inner_check all_check" />
								...
								<tbody>
									<tr id='trmain'></tr>
									<tr v-for="v in evtList" :key="v.idx" :id="v.eventId" :tr-id='v.trId'>
                                        <td>
                                            <span v-html="v.ckbtn" :data-evt-id='v.eventId' :id="v.eventId" :value='v.eventId'></span>
                                        </td>
                                        <td>
                                            <span>{{v.aggName}}</span>
                                        </td>
                                        ...
                                        <td>
                                            <span v-on:click="hrefAreaCtrl($event,v.aggId,v.siteId)" :id='v.aggId' :value='v.siteId' v-html="v.btn"></span>
                                        </td>
									</tr>
								</tbody>
							</table>
...
export default {
	...
	methods:{
        ...
        getEventListByrcc(rcc){
            ...
            var item = " where "+ _wherecondition('rcc', rcc, null) + ' and   Ackts is null'
          
            var query = 'SELECT *'
            +' FROM mysql.peiuoperation.`vwEventRecord` '+ item 
            ...
            for(var i = 0; i< data.length; i++){
                ...
                this.evtList.push({
                    ...
                    ckbtn : "<input type='checkbox' name='aaaa' data-evt-id='"+eventid+"' id='"+eventid+"' class='tbl_inner_check'><label for='tbl_check0'></label>",
                    btn: "<button type='button' class='btn btn_tbl_inner btn_management'><span class='hide'>계정관리</span></button>"
                })
            }
        }, 
        getHashMapbysiteid(){
